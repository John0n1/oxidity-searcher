...
pub struct StrategyExecutor {
    pub(super) tx_rx: Mutex<UnboundedReceiver<StrategyWork>>,
    pub(super) mut_block_rx: Mutex<Receiver<Header>>,
    pub(super) safety_guard: Arc<SafetyGuard>,
    pub(super) bundle_sender: SharedBundleSender,
    pub(super) db: Database,
    pub(super) portfolio: Arc<PortfolioManager>,
    pub(super) gas_oracle: GasOracle,
    pub(super) price_feed: PriceFeed,
    pub(super) chain_id: u64,
    pub(super) stats: Arc<StrategyStats>,
    pub(super) max_gas_price_gwei: u64,
    pub(super) simulator: Simulator,
    pub(super) token_manager: Arc<TokenManager>,
    pub(super) signer: PrivateKeySigner,
    pub(super) nonce_manager: NonceManager,
    pub(super) slippage_bps: u64,
    pub(super) http_provider: HttpProvider,
    pub(super) dry_run: bool,
    pub(super) router_allowlist: HashSet<Address>,
    pub(super) wrapped_native: Address,
    pub(super) inventory_tokens: DashSet<Address>,
    pub(super) last_rebalance: Mutex<Instant>,
    pub(super) toxic_tokens: DashSet<Address>,
    pub(super) executor: Option<Address>,
    pub(super) executor_bribe_bps: u64,
    pub(super) executor_bribe_recipient: Option<Address>,
    pub(super) flashloan_enabled: bool,
    pub(super) reserve_cache: Arc<ReserveCache>,
    pub(super) bundle_state: Arc<Mutex<Option<BundleState>>>,
    pub(super) v3_quote_cache: DashMap<B256, V3QuoteCacheEntry>,
    pub(super) current_block: AtomicU64,
    pub(super) sandwich_attacks_enabled: bool,
    pub(super) simulation_backend: String,
}

impl StrategyExecutor {
    pub(super) fn is_nonce_gap_error(err: &AppError) -> bool {
    ...
    pub(super) fn log_skip(&self, reason: &str, detail: &str) {
        if self.dry_run {
    ...
    pub(super) fn amount_to_display(&self, amount: U256, token: Address) -> f64 {
        let decimals = self
            .token_manager
    ...
    pub(super) async fn probe_v3_sell_for_toxicity(
        &self,
        router: Address,
        path: Vec<u8>,
        amount_in: U256,
    ...
    pub(super) async fn await_receipt(&self, hash: &B256) -> Result<bool, AppError> {
        for _ in 0..3 {
...
mod tests {
    ...
    use crate::services::strategy::decode::{
        decode_swap_input, direction, parse_v3_path, target_token, v3_fee_sane, RouterKind,
        ObservedSwap, SwapDirection,
    };
    ...
    fn decodes_uniswap_v3_exact_input_single() {
        ...
        let data = call.abi_encode();
        let decoded = decode_swap_input(WETH_MAINNET, &data, U256::from(0u64))
