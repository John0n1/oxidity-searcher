...
impl StrategyExecutor {
    ...
    pub async fn rebalance_token(&self, token: Address, router: Address) -> Result<(), AppError> {
        ...
        let mut gas_fees: crate::network::gas::GasFees = self.gas_oracle.estimate_eip1559_fees().await?;
        ...
        let sell_amount = bal;
        let expected_out = if let Some(q) = self.reserve_cache.quote_v2_path(&sell_path, sell_amount) {
            q
        } else {
        ...
        let access_list = self
            .apply_access_list(&mut request, StrategyExecutor::build_access_list(router, &[token]))
            .await;
    ...
    pub async fn probe_v2_sell_for_toxicity(
        ...
        match UniV2Router::swapExactTokensForETHCall::abi_decode_returns(&outcome.return_data) {
            Ok(amounts) => {
                ...
                let tolerance_bps = U256::from(10_000u64 - crate::services::strategy::strategy::TAX_TOLERANCE_BPS);
