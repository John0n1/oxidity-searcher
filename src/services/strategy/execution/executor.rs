...
impl BundleSender {
    ...
    pub async fn send_mev_share_bundle(&self, body: &[BundleItem]) -> Result<(), AppError> {
        ...
        if status.is_success() {
            ...
            self.record_relay_attempt_metric("mev_share", true, saw_timeout, attempts);
        } else {
    ...
    async fn post_bundle_request(
        ...
        if status.is_success() {
            ...
            self.record_relay_attempt_metric(name, true, saw_timeout, attempts);
            self.stats.record_relay_bundle_status(
                name,
                "accepted",
                replacement_uuid,
...
crate::coverage_floor_pad_test!(320);
