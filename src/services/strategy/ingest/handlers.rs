...
use crate::services::strategy::decode::{
    SwapDirection, decode_swap, decode_swap_input, direction, target_token,
};
...
impl StrategyExecutor {
    ...
    async fn evaluate_mempool_tx(&self, tx: &Transaction) -> Result<Option<String>, AppError> {
        ...
        let reserved_base = self.lease_nonces(3).await?;
        let mut nonce_cursor = reserved_base;
        ...
        if self.executor.is_some() && front_run.is_none() {
            if let Some(exec_addr) = self.executor {
                ...
                if !targets.is_empty() {
                    ...
                    let exec_call =
                        crate::data::executor::UnifiedHardenedExecutor::executeBundleCall {
                            targets,
                            payloads,
                            values,
