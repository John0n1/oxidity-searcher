...
S_{\max}
=
\frac{\mathrm{max\_bps} \cdot R_1}
{10000 - \mathrm{max\_bps}}
```
---
Where:
...
| $\mathrm{max_bps}$ | Configured price-impact limit, in basis points |
| $R_1$              | Relevant pool reserve                          |
| $S^*$              | Estimated optimal profitable size              |

#### The selected candidate size is therefore bounded by:
---
```math
S^* \leq S_{\max}
```
...
| Recommended distributions | Debian, Arch Linux |
| Rust toolchain | Latest |
| Build tools | `cargo`, `rustc` |
| Execution node | Local Reth or Nethermind |
| Database | SQLite 3 |
...
**Built with Rust for low-latency EVM execution.**
